...
notcancelledflights = df_air_time.filter( (df_air_time.CANCELLED == 0)
 
print "Fights which dont cancel"
notcancelledflightscarrier = notcancelledflights .groupby('UNIQUE_CARRIER').count().orderBy('count', ascending=False).limit(5)
notcancelledflightscarrierjoined = notcancelledflightscarrier.join(df_carrier, notcancelledflightscarrier.UNIQUE_CARRIER == df_carrier.Code)
print notcancelledflightscarrierjoined.show()
