...
print "5) Top 5 cancellation flights per DayOfWeek "

cancelFlightsBadWeather = df_air_time.filter( (df_air_time.CANCELLED == 1))
mostcancelFlightsWeek = cancelFlightsBadWeather.groupby('DAY_OF_WEEK', 'FL_NUM').count().orderBy('count', ascending=False).limit(5)
print mostcancelFlightsWeek.show()
