...
print "top 10 states having highest number of airports"
top10states = df_airports.filter( (df_airports.state.isNotNull()) )
top10statesgrp = top10states.groupby('state').count().orderBy('count', ascending=False).limit(10)
print top10statesgrp.select('state').collect()
