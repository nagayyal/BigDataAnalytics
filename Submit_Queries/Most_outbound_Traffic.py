...
print "top 5 Most-OUTBOUND_Traffic:"
top5outtrf = df_air_time.filter( (df_air_time.ORIGIN.isNotNull())  )
top5outtrfgrp = top5outtrf.groupby('ORIGIN').count().orderBy('count', ascending=False).limit(5)
print top5outtrfgrp.select('ORIGIN', 'count').collect()
