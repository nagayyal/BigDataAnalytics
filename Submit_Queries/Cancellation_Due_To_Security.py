...
print "2) Top cancellation flight per month due to Security"
 
 
cancelFlightsBadWeather = df_air_time.filter( (df_air_time.CANCELLED == 1) & (df_air_time.CANCELLATION_CODE == 'D'))
 
print "Top cancellation flight per month due to Security"
mostcancelFlightsBadWeather = cancelFlightsBadWeather.groupby('MONTH', 'FL_NUM').count().orderBy('count', ascending=False).limit(5)
print mostcancelFlightsBadWeather.show()
