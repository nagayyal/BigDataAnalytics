...
print "top 5 busiest airport routes (Origin, Dest):"
top5busyairportroutes = df_air_time.filter( (df_air_time.ORIGIN.isNotNull()) & (df_air_time.DEST.isNotNull()) )
top5busyairportroutesgrp = top5busyairportroutes.groupby('ORIGIN', 'DEST').count().orderBy('count', ascending=False).limit(10)
print top5busyairportroutesgrp.select('ORIGIN', 'DEST', 'count').collect()
