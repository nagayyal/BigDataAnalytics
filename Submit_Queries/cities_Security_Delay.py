...
print "2) Cities having Security delay"
 
 
AirportsSecurityflightdelay = df_air_time.filter((df_air_time.SECURITY_DELAY > 0))
 
print "Airports having Security delay"
statesNASdelay = AirportsSecurityflightdelay.groupby('ORIGIN_AIRPORT_ID').count().orderBy('count', ascending=False).limit(5)
