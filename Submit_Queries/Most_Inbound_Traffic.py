...
print "top 5 Most-Inbound_Traffic:"
top5inbtrf = df_air_time.filter( (df_air_time.DEST.isNotNull())  )
top5inbtrfgrp = top5inbtrf.groupby('DEST').count().orderBy('count', ascending=False).limit(5)
print top5inbtrfgrp.select('DEST', 'count').collect()
