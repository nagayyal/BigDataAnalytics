...
print "5) Carriers which had most cancellations "

cancelFlights = df_air_time.filter( (df_air_time.CANCELLED == 1))
mostcancelCarrier = cancelFlights.groupby('UNIQUE_CARRIER').count().orderBy('count', ascending=False).limit(5)
mostcancelCarrierjoined = mostcancelCarrier.join(df_carrier, mostcancelCarrier.UNIQUE_CARRIER == df_carrier.Code)
print nmostcancelCarrierjoined.show()
