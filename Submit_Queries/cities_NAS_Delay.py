...
print "2) Cities having NAS delay"
 
 
AirportsNASflightdelay = df_air_time.filter((df_air_time.NAS_DELAY > 0))
 
print "Airports having NAS delay"
statesNASdelay = NASflightdelay.groupby('ORIGIN_AIRPORT_ID').count().orderBy('count', ascending=False).limit(5)
