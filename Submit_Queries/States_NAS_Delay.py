...
print "2) States having NAS delay"
 
 
NASflightdelay = df_air_time.filter((df_air_time.NAS_DELAY > 0))
 
print "States having NAS delay"
statesNASdelay = NASflightdelay.groupby('ORIGIN_STATE_ABR').count().orderBy('count', ascending=False).limit(5)
print statesNASdelay.select('ORIGIN_STATE_ABR','count').show()
